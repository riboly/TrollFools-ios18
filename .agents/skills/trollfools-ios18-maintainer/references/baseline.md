...
- `4.3-269`: **REVERTED**. Its TrollFools-start trust-restoration workaround is unnecessary with the user's updated Dopamine RootHide. The user subsequently verified that Build 268 survives a full restart and launches the injected App directly, without opening TrollFools first. The exact updated Dopamine RootHide version was not recorded.
