...
- `4.3-267`: replaces the incorrect custom-trust entitlement backend with `RootHide trust-cache`. It dynamically maps `libjailbreak.dylib`, verifies `jbclient_trust_executable_recurse` belongs to that image, preserves entitlements, ad-hoc signs each modified Mach-O, and uploads its CDHash through the RootHide-specific recursive trust domain. Dry Run resolves the same capability and signs temporary copies but intentionally does not mutate the kernel trust cache. It is **STATICALLY VERIFIED** until the exact package is installed and tested.
...
- Dopamine RootHide trust-cache capability requires dynamically mapped `libjailbreak.dylib` and a `jbclient_trust_executable_recurse` symbol whose image origin matches that library. Signing preserves the original entitlement dictionary, runs `ldid`, then calls the recursive trust API for the signed Mach-O.
- `jb.pmap_cs.custom_trust=PMAP_CS_APP_STORE` is not a framework or dylib signing mechanism. RootHide's process-start hook reads it only from the main executable and changes only the main CodeDirectory trust. The systemwide trust-file path also skips removable App Store bundle paths without a TrollStore Lite marker, while the RootHide recursive-trust domain directly collects and uploads the required CDHashes.
