...
- `4.3-269`: restores missing dynamic RootHide trust when TrollFools starts. It discovers only TrollFools-managed Mach-Os from backups, added load commands, the deferred-loader manifest, and known support assets; it backs up the per-App group, reuses hidden staging registration, and rolls the group back on failure. This does not run during Dopamine activation and must not be described as boot-time trust persistence.
