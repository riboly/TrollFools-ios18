...
- `4.3-269`: **DEVICE VERIFIED for app-start RootHide trust restoration in the reported Telegram/Lead workflow**. Restoring the four managed files to unregistered pre-recovery hashes reproduced a dyld initialization crash. Starting TrollFools restored the four preferred-architecture CDHashes through hidden staging; Telegram then remained alive and Frida showed `TrollFoolsLoader.dylib` plus `Lead1.44.dylib` loaded without reinjection. It discovers only TrollFools-managed Mach-Os from backups, added load commands, the deferred-loader manifest, and known support assets; it backs up the per-App group and rolls the group back on failure. This does not run during Dopamine activation and must not be described as boot-time trust persistence.
