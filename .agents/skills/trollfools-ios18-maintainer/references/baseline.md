...
- `4.3-263`: system dyld-cache alias detection and plug-in `/usr/lib` rpath normalization for `@rpath/<leaf>.dylib` dependencies. Created from HBWechatHelper and MikotoHelper reports whose public iOS system libraries were incorrectly treated as missing App-private assets. **STATICALLY VERIFIED** until both reported injections are tested on the primary device.
