...
- `4.3-266`: adds a separately reported `RootHide custom-trust` backend. It requires validated `jbroot`, mapped `ldid`, TrollStore Lite registration, and both custom-trust markers in the installed helper; it preserves existing entitlements, adds the single required trust entitlement, signs, reads it back, and exercises the same path in Dry Run. Bootstrap `fastPathSign`, rootless ad-hoc, and ChOma behavior remain available. It is **STATICALLY VERIFIED** until the exact package is installed and tested.
...
- Dopamine RootHide custom-trust capability additionally requires an installed TrollStore Lite helper containing both `jb.pmap_cs.custom_trust` and `PMAP_CS_APP_STORE`. Signing preserves the original entitlement dictionary, adds that one key/value, runs `ldid`, and validates the embedded result. This mirrors the Dopamine 2.1.5+ trust-level mechanism used by the installed helper without copying or invoking that helper.
- If RootHide capability selection falls back to ChOma, the report must list TrollStore Lite registration, rootless `ldid`, mapped RootHide `ldid`, mapped `fastPathSign`, and custom-trust marker status.
