...
9. For RootHide support, confirm the source contains separate `RootHide fast-path` and `RootHide trust-cache` backends. The packaged main executable must contain RootHide capability diagnostics, `fastPathSign`, `jbclient_trust_executable_recurse`, `/usr/lib/libjailbreak.dylib`, and `/usr/lib/libroothide.dylib`; Swift may not emit enum raw-value strings contiguously. Confirm source and documentation contain no device-specific `.jbroot-*` value and that framework/dylib signing does not add `jb.pmap_cs.custom_trust`.
