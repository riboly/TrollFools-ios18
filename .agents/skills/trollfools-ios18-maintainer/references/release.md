...
9. For RootHide support, confirm the source contains the `RootHide ad-hoc` backend and the packaged main executable contains the RootHide capability diagnostic plus `/usr/lib/libroothide.dylib`; Swift may not emit an enum raw-value string contiguously. Confirm source and documentation contain no device-specific `.jbroot-*` value.
