...
If an injected App fails only after a full restart, first record the exact Dopamine RootHide version and compare the current CDHash with `jbctl trustcache info`. Updated Dopamine RootHide builds may restore the required trust during environment activation; Build 268 has been device-verified to launch directly after restart in the user's updated environment. Do not reintroduce TrollFools app-start restoration unless missing trust is reproduced on the current Dopamine build.
