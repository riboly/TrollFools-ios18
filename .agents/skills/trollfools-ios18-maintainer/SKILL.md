...
- Treat `4.3-264` as device-failed for RootHide injection: Telegram 12.9.2 crashed at dyld launch because `ldid -S` alone did not establish the required trust level. Treat `4.3-265` as device-failed for Dopamine RootHide capability selection because the device has no runtime `fastPathSign` and the app fell back to ChOma. Treat `4.3-266` as device-failed because adding `jb.pmap_cs.custom_trust` to injected frameworks did not add their CDHashes to the jailbreak trust cache. Treat `4.3-267` as device-failed because the recursive collector silently skipped an App Store framework without `_TrollStoreLite`. Treat `4.3-268` as statically verified until its exact package is installed; the underlying trust-cache mechanism was separately proven on the device with Telegram.
