...
- Build `4.3-269` is device-verified for app-start restoration of dynamic RootHide trust in the reported Telegram/Lead workflow. Restoring the four managed files to unregistered pre-recovery hashes reproduced the dyld launch failure; starting TrollFools restored all four preferred-architecture hashes through hidden staging, and Telegram then remained running with the Loader and Lead loaded without reinjection. Enumerate only TrollFools-managed Mach-Os, back up the entire per-App target group first, and roll that group back if any target fails. Do not describe this as boot-time persistence: the user must start TrollFools before starting an affected injected App unless Dopamine independently implements boot-time restoration.
