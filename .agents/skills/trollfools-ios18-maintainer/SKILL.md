...
- Treat `4.3-264` as device-failed for RootHide injection: Telegram 12.9.2 crashed at dyld launch because `ldid -S` alone did not establish the required trust level. Treat `4.3-265` as device-failed for Dopamine RootHide capability selection because the device has no runtime `fastPathSign` and the app fell back to ChOma. Treat `4.3-266` as device-failed because adding `jb.pmap_cs.custom_trust` to injected frameworks did not add their CDHashes to the jailbreak trust cache. Treat `4.3-267` as statically verified until its recursive-trust backend is installed and tested.
...
- Never add `jb.pmap_cs.custom_trust` to a framework or dylib as a trust mechanism. RootHide consumes it from the process main executable only. Its systemwide trust-file path intentionally skips App Store removable bundles without a TrollStore Lite marker, so use the RootHide recursive-trust domain instead. Dry Run must validate the API without uploading temporary CDHashes.
...
5. For RootHide support, verify the packaged main executable contains diagnostics for both `fastPathSign` and `jbclient_trust_executable_recurse`; when the deferred loader is present, also verify arm64+arm64e slices, `@rpath/TrollFoolsLoader.dylib`, the `__interpose` section, and the packaged reentrancy-guard diagnostic string.
