...
- `4.3-269`：**已撤销**。该版本加入的 TrollFools 启动时信任恢复属于对旧版 Dopamine RootHide 行为的应用层规避。用户升级 Dopamine RootHide 后确认 `4.3-268` 在完整重启并重新月余后可以直接启动已注入 App，无需先打开 TrollFools，因此仓库代码和产品版本恢复为 `4.3-268`。新版 Dopamine RootHide 的精确版本号尚未记录。
...
若故障只在完整重启后出现，先记录当前 Dopamine RootHide 精确版本，并比较目标文件 CDHash 与 `jbctl trustcache info`。新版 Dopamine RootHide 已在用户设备上验证可在环境激活阶段恢复所需信任，`4.3-268` 无需 TrollFools 启动时恢复；除非当前环境重新证明信任条目缺失，否则不要恢复 269 的应用层规避代码。
