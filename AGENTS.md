...
- `4.3-267` is device-failed because RootHide's recursive trust collector also skips removable App Store bundles without a container-level `_TrollStoreLite` marker while still returning success. `4.3-268` trusts a hidden staging copy, copies the randomized signature back, and verifies the final CDHash through dynamically mapped `jbctl trustcache info` without creating that marker. Treat 268 as statically verified until the exact package is installed and tested; the underlying trust-cache mechanism was device-verified manually with Telegram.
