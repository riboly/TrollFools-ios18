...
- `4.3-269` restores missing RootHide trust for TrollFools-managed Mach-Os when TrollFools starts. A full device restart removes dynamic trust-cache entries even though injected App files remain; the app enumerates only its managed targets, restores them through the validated hidden staging path, and rolls the whole per-App file group back on failure. This is an app-start recovery path, not boot-time persistence.
