...
- `4.3-267` is device-failed because RootHide's recursive trust collector also skips removable App Store bundles without a container-level `_TrollStoreLite` marker while still returning success. `4.3-268` is device-verified on iPhone XS Max/iOS 18.2.1/Dopamine RootHide 3.0.23 for the reported Telegram 12.9.2 and Lead1.44 workflow: hidden staging, final trust-cache validation, injection, and subsequent app launch all succeeded without creating that marker. Do not generalize this result to arbitrary apps or plug-ins.
- `4.3-269` is device-verified for app-start recovery of missing RootHide trust for the reported Telegram/Lead workflow. With unchanged injected files restored to their pre-registration hashes, Telegram reproduced the dyld launch failure; opening TrollFools 269 restored all managed preferred-architecture CDHashes through hidden staging, after which Telegram stayed running and Frida confirmed the Loader and Lead were loaded without reinjection. This is an app-start recovery path, not boot-time persistence.
...
- Do not use `jb.pmap_cs.custom_trust` as a framework or dylib signing mechanism. RootHide reads that entitlement from the process main executable only. Both RootHide trust collectors skip App Store bundle paths without a container-level `_TrollStoreLite` marker. Trust a signed copy under the validated hidden temporary root, copy the randomized result back transactionally, and verify the final CDHash in the jailbreak trust cache rather than creating a detectable marker or trusting a zero API result.
- The user has persistently authorized OpenSSH and Frida for this device, including process enumeration, attach, spawn, script loading, debug injection, and starting or terminating target apps. These operations do not require repeated approval while they stay within this repository's maintenance and diagnosis scope.
- The user has also authorized changes to ordinary App Store app or TrollFools containers, injected plug-ins, injection backups, and validated RootHide hidden temporary directories, including installing and testing TrollFools TIPA builds. Preserve transactional backup, rollback, and exact-path validation.
- Never modify, replace, or delete system/rootfs, system partition, bootstrap, preboot, launchd, Dopamine, or RootHide core files or boot configuration without new operation-specific authorization. Treat `/System`, `/bin`, `/sbin`, system `/usr`, `/Library`, and `/basebin` as protected. Do not update `jbctl`, clear or rebuild trust caches, install packages into protected paths, restart system services, perform a userspace reboot, or reboot the device. Normal TrollFools temporary trust registration for an injected app and read-only trust-cache inspection remain allowed.
